...
use crate::garlic::state::clove_cache::{CloveCache, SerializableCloveCache};
use crate::garlic::state::request_state::{
    InitiatorRequest, Proxy, ProxyRequest, SerializableInitiatorRequest, SerializableProxy,
    SerializableProxyRequest,
};
use crate::garlic::{Clove, CloveNode, CloveRequestID, GarlicMessage};
use crate::net::message_handler::{GMessage, GarlemliaMessageHandler};
...
impl SerializableGarlicCast {
    pub fn from(garlic: GarlicCast) -> SerializableGarlicCast {
        SerializableGarlicCast {
            ...
            requests_as_initiator: SerializableInitiatorRequest::hashmap_to_serializable(
                garlic.requests_as_initiator.clone(),
            ),
            requests_as_proxy: SerializableProxyRequest::hashmap_to_serializable(
                garlic.requests_as_proxy.clone(),
            ),
    ...
    pub fn to_garlic(self, socket: UdpSocket) -> GarlicCast {
        GarlicCast {
            ...
            requests_as_initiator: SerializableInitiatorRequest::hashmap_to_initiator_request(
                self.requests_as_initiator,
            ),
            requests_as_proxy: SerializableProxyRequest::hashmap_to_proxy_request(
                self.requests_as_proxy,
            ),
...
pub struct GarlicCast {
    ...
    pub(crate) local_node: Node,
    pub(crate) message_handler: Arc<Box<dyn GMessage>>,
    pub known_nodes: Vec<Node>,
    ...
    pub(crate) public_key: Option<RsaPublicKey>,
...
impl GarlicCast {
    ...
    pub(crate) fn create_initiator_request(&self, request_id: U256) -> InitiatorRequest {
        InitiatorRequest {
            request_id,
    ...
    pub async fn send_to_proxy(&mut self, mut proxy: Proxy, cloves: Vec<Clove>) -> bool {
        ...
        let n_1_good =
            Forwarding::forward_from_proxy(self, &n_1_clove_node, &cloves[0], &proxy).await;
        ...
        if n_1_good.is_err() {
            ...
            ProxyDiscovery::remove_proxy(self, &proxy);
        ...
        if n_1_replaced.is_some() {
            ...
            ProxyDiscovery::replace_proxy(self, &proxy, &new_proxy_n_1);
            proxy = new_proxy_n_1;
        }

        let n_2_good =
            Forwarding::forward_from_proxy(self, &n_2_clove_node, &cloves[1], &proxy).await;
        ...
        if n_2_good.is_ok() {
            ...
            if n_2_replaced.is_some() {
                ...
                ProxyDiscovery::replace_proxy(self, &proxy, &new_proxy_n_2);
            }
            true
        } else {
    ...
    pub(crate) async fn replace_with_alt(
        &mut self,
        next_node: CloveNode,
        mut alt_msg: GarlicMessage,
    ) -> Result<CloveNode, ()> {
        ...
        if let Err(e) = self.send_forward_message(&updated_node, &alt_msg).await {
            eprintln!(
                ...
                updated_node.node.address, e
            );
        }

        match self
            .message_handler
            .recv(200, &updated_node.node.address)
            .await
        {
