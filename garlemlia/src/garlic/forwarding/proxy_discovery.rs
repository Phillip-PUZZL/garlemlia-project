use crate::core::constants::{MIN_PROXY_COUNT, PROXY_REQUEST_MESSAGE, PROXY_RESPONSE_TIMEOUT_MS};
...
pub(crate) trait TraitProxyDiscovery {
    #[allow(dead_code)]
    async fn discover_proxies(garlic: &mut GarlicCast, count: u8);
    #[allow(dead_code)]
    ...
    #[allow(dead_code)]
    fn update_cache(garlic: &mut GarlicCast, new_clove: CloveNode, sequence_number: U256);
    fn replace_proxy(garlic: &mut GarlicCast, old_proxy: &Proxy, new_proxy: &Proxy);
    fn remove_proxy(garlic: &mut GarlicCast, proxy: &Proxy);
    fn remove_proxy_from_requests(garlic: &mut GarlicCast, proxy: &Proxy);
    fn remove_proxy_from_initiator_requests(garlic: &mut GarlicCast, proxy: &Proxy);
    fn filter_proxies(garlic: &GarlicCast, proxy_id_pool: Vec<U256>) -> Vec<Proxy>;
}

pub struct ProxyDiscovery;
impl TraitProxyDiscovery for ProxyDiscovery {
    async fn discover_proxies(garlic: &mut GarlicCast, count: u8) {
        ...
        let mut available_nodes = garlic.known_nodes.clone();
        garlic.do_not_forward.insert(sequence_number, Utc::now());

        let mut pending_cloves =
            CloveProtocol::generate(proxy_request, target_count, sequence_number, None, None)
                .unwrap();
        ...
        while successful_proxies < target_count {
            ...
            for task in discovery_tasks {
                match Self::handle_discovery_result(garlic, task.await, sequence_number).await {
                    Ok(()) => successful_proxies += 1,
                    Err(failed_attempt) => {
                        pending_cloves.push(failed_attempt.clove);
                        garlic.known_nodes.retain(|x| *x != failed_attempt.node);
                    }
                }
            }
        }
    ...
    async fn handle_discovery_result(
        ...
        match task_result {
            Ok(Ok(successful_node)) => {
                ...
                Self::update_cache(garlic, new_clove, sequence_number);
                Ok(())
            }
            Ok(Err((failed_node, clove))) => Err(ProxyDiscoveryResult {
                node: failed_node,
                clove,
            }),
            Err(e) => {
        ...
        for _ in 0..count {
            ...
            tasks.push(tokio::spawn(async move {
                ...
                match message_handler
                    .recv(PROXY_RESPONSE_TIMEOUT_MS, &target_node.address)
                    .await
                {
                    Ok(_) => Ok(target_node),
    ...
    fn update_cache(garlic: &mut GarlicCast, new_clove: CloveNode, sequence_number: U256) {
        garlic.cache.insert_next_hop(new_clove.clone(), None);
        garlic.cache.insert_association(sequence_number, new_clove);
    ...
    /// # Example
    ///
    /// // Given `proxies`, `initiators`, and request mappings in garlic:
    /// // Assume `old_proxy` has sequence_number = 42 and `new_proxy` is a replacement proxy.
    /// garlic.replace_proxy(&old_proxy, &new_proxy);
    ///
    /// // Post execution:
    ...
    fn replace_proxy(garlic: &mut GarlicCast, old_proxy: &Proxy, new_proxy: &Proxy) {
        ...
        replace_in_vec(&mut garlic.proxies, old_proxy, new_proxy);
        replace_in_vec(&mut garlic.initiators, old_proxy, new_proxy);

        // Update proxy requests
        let updates: Vec<_> = garlic
            .requests_as_proxy
            .iter()
        ...
        for (request_id, updated_request) in updates {
            garlic.requests_as_proxy.insert(request_id, updated_request);
        }
        ...
        let updates: Vec<_> = garlic
            ...
            .filter(|(&_request_id, initiator_request)| {
                initiator_request
                    .proxies
                    .iter()
                    .any(|p| p.sequence_number == old_proxy.sequence_number)
            })
            .map(|(&request_id, initiator_request)| {
                let mut updated_request = initiator_request.clone();
                if let Some(index) = updated_request
                    .proxies
                    .iter()
                    .position(|p| p.sequence_number == old_proxy.sequence_number)
                {
    ...
    ///   If found, it is removed from the collection.
    /// - Calls `garlic.remove_proxy_from_requests()` to handle removal of the proxy from requests.
    /// - Calls `Self::remove_proxy_from_initiator_requests(garlic, )` to handle removal of the proxy from
    ...
    fn remove_proxy(garlic: &mut GarlicCast, proxy: &Proxy) {
        if let Some(index) = garlic
            .proxies
            .iter()
            .position(|p| p.sequence_number == proxy.sequence_number)
        {
            garlic.proxies.remove(index);
        }
        if let Some(index) = garlic
            .initiators
            .iter()
            .position(|p| p.sequence_number == proxy.sequence_number)
        {
            garlic.initiators.remove(index);
        }
        Self::remove_proxy_from_requests(garlic, proxy);
        Self::remove_proxy_from_initiator_requests(garlic, proxy);
    }
    ...
    fn remove_proxy_from_requests(garlic: &mut GarlicCast, proxy: &Proxy) {
        let keys_to_remove: Vec<U256> = garlic
            .requests_as_proxy
            .iter()
        ...
        for key in keys_to_remove {
            garlic.requests_as_proxy.remove(&key);
        }
    }
    ...
    fn remove_proxy_from_initiator_requests(garlic: &mut GarlicCast, proxy: &Proxy) {
        let updates: Vec<_> = garlic
            ...
            .filter_map(|(request_id, request)| {
                if let Some(proxy_index) = request
                    .proxies
                    .iter()
                    .position(|p| p.sequence_number == proxy.sequence_number)
                {
