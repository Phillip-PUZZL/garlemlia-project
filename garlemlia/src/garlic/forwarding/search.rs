use crate::core::constants::{DEFAULT_TTL, MIN_SEARCH_COUNT};
...
#[allow(dead_code)]
pub(crate) trait TraitSearch {
    async fn search_kademlia(garlic: &mut GarlicCast, proxy_id_pool: Vec<U256>, key: U256);
    ...
    async fn search_overlay(
        ...
        req: String,
        proxy_id_pool: Vec<U256>,
        count: u8,
    );
}

pub struct Search;
impl TraitSearch for Search {
    ...
    /// - The `proxy_id_pool` is filtered to retain proxies that are present in the object's proxy pool (`garlic.proxies`).
    ...
    /// garlic.search_kademlia(proxy_id_pool, key_to_search).await;
    ///
    /// # Panics
    ...
    ///   is within bounds.
    /// - Any updates to `garlic.proxies` should ensure thread safety, as clones are explicitly created
    ...
    /// - The `garlic.requests_as_initiator` map is updated to track active and completed requests.
    ///
    /// # Limitations
    ...
    async fn search_kademlia(garlic: &mut GarlicCast, proxy_id_pool: Vec<U256>, key: U256) {
        ...
        while !sent && proxies.len() > 0 {
            ...
            let cloves = CloveProtocol::generate(
                ...
                Some(CloveRequestID::new(request_id, 0)),
            )
            .unwrap();
    ...
    async fn spawn_search_tasks(
        ...
        for i in 0..remaining {
            ...
            let msg = Self::create_search_message(garlic, req, &request_id_full, proxy_id);
    ...
    async fn search_overlay(
        ...
        let mut proxies = ProxyDiscovery::filter_proxies(garlic, proxy_id_pool);
