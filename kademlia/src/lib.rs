...
impl Kademlia {
    ...
    pub async fn iterative_find_node(&self, socket: Arc<UdpSocket>, target_id: u128) -> Vec<Node> {
        ...
        closest_nodes.retain(|x| *x != self.node);
        ...
        while new_nodes_found {
            ...
            if new_nodes_found {
                new_closest_nodes.retain(|x| *x != self.node);
                closest_nodes.clear();
        ...
        all_nodes.push(self.node.clone());
    ...
    pub async fn iterative_find_value(&self, socket: Arc<UdpSocket>, key: u128) -> Option<String> {
        ...
        let mut queried_nodes = HashSet::new();
        queried_nodes.insert(self.node.address);
...
mod tests {
    ...
    async fn test_iterative_find_node() {
        ...
        assert!(!found_nodes.is_empty() && found_nodes[0] == node2.node, "Should find node 2 and it should be first in the list");
    }
    ...
    async fn test_add_node_ping() {
        ...
        kad.add_node(&kad.socket, test.node.clone()).await;
        for node in &nodes {
            kad.add_node(&kad.socket, node.clone()).await;
        }
        ...
        assert_eq!(new.nodes[DEFAULT_K - 1], test.node, "Bucket should have same nodes");
