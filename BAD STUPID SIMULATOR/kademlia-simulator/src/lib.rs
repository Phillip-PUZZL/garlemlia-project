...
use kademlia_structs::{HeapNode, KBucket, KMessage, KademliaMessage, KademliaRoutingTable, MessageChannel, MessageHandler, Node, RoutingTable, DEFAULT_K};
...
impl KMessage for SimulatedMessageHandler {
    ...
    async fn send(&self, socket: &UdpSocket, target: &SocketAddr, msg: &KademliaMessage) {
        ...
        //println!("send - sim :: data of length {} to {} from {}", message_bytes.len(), target, socket.local_addr().unwrap());
...
impl Simulator {
    ...
    pub async fn add_node(&self, address: SocketAddr) -> Option<SocketAddr> {
        ...
        if !started {
            ...
            let kad;
            {
                kad = self.unavailable.lock().await.get(&address).cloned();
            }

            match kad {
                Some(val) => {
                    Some(val.receive_addr)
                }
                _ => {
                    None
                }
...
pub async fn create_network(mut nodes: Vec<SimulatedNode>) {
    ...
    {
        SIM.lock().await.set(Simulator::new(8000, 100, vec![nodes[0].clone()]).await);
    }
...
pub async fn create_random_simulated_nodes(count: u16) -> Vec<SimulatedNode> {
    ...
    for i in 0..count {
        ...
        simulated_nodes.push(SimulatedNode {
            node: node.clone(),
            routing_table: SimulatedRoutingTable {local_node: node.clone(), buckets: HashMap::new()},
...
mod sim_tests {
    ...
    async fn simulated_node_thread_test() {
        let file_path = "../kademlia_nodes_realistic.json";
        ...
        match load_simulated_nodes(file_path).await {
            Ok(mut nodes) => {
                ...
                let node1 = create_test_node(rand::random::<u128>(), 6000).await;

                {
                    println!("NODE ID USED: {}", node1.node.lock().await.id);
                }
