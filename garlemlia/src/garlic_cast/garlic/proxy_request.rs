...
pub(crate) trait ProxyRequest {
    // validator pool creations or searches
    async fn run_proxy_message(&mut self, req: CloveMessage, response: Option<GarlemliaResponse>) -> Option<GarlemliaMessage>;
    async fn manage_proxy_message(&self, req: CloveMessage) -> Option<CloveMessage>;
    async fn accept_proxy(&mut self, sequence_number: U256, second_clove: Clove, node: Node) -> Result<Proxy, MessageError>;
    fn update_proxy(&mut self, old_proxy: Proxy, new_proxy: Proxy) -> Proxy;
    async fn handle_proxy_forwarding(&mut self, forward_type: u8, proxy: Proxy, cloves: Vec<Clove>) -> Option<Proxy>;
...
impl ProxyRequest for GarlicCast {
    ...
    ///
    /// # Parameters
    /// - `self`: A mutable reference to the current instance of the validator pool.
    ...
    /// - Uses `self.proxies` and `self.requests_as_proxy` for proxy selection and request management.
        ...
        match req.clone() {
            CloveMessage::SearchOverlay { request_id, proxy_id, public_key, search_term, ttl } => {
                ...
                if response.is_some() {
                    ...
                    if current_request.is_some() {
                        ...
                        cloves = GarlicCast::generate_cloves(msg.clone(), 2, proxy.sequence_number, Some(RsaPublicKey::from_public_key_pem(&*public_key).unwrap()), Some(request_id.clone())).unwrap();
                    } else {
                        ...
                        let res_cloves = GarlicCast::generate_cloves(res_msg.clone(), 2, request_id.request_id, Some(RsaPublicKey::from_public_key_pem(&*public_key).unwrap()), Some(request_id.clone())).unwrap();
                        ...
                        cloves = GarlicCast::generate_cloves(msg.clone(), 2, proxy.sequence_number, Some(proxy.clone().public_key), Some(CloveRequestID::new(u256_random(), rand::random::<u64>()))).unwrap();
                    }
            ...
            CloveMessage::SearchGarlemlia { request_id, .. } => {
                ...
                if current_request.is_some() {
                    ...
                    let cloves = GarlicCast::generate_cloves(msg.clone(), 2, proxy.sequence_number, Some(proxy.clone().public_key), Some(request_id.clone())).unwrap();
    ...
    async fn accept_proxy(&mut self, sequence_number: U256, second_clove: Clove, node: Node) -> Result<Proxy, MessageError> {
        ...
        let msg_from_initiator_check = GarlicCast::reconstruct_clove_message(first_clove.clone().clove, second_clove.clone());
    ...
    async fn handle_proxy_forwarding(&mut self, forward_type: u8, mut proxy: Proxy, cloves: Vec<Clove>) -> Option<Proxy> {
        ...
        if send_error_neighbor1(forward_type) {
            match self.process_neighbor_failure(&first_neighbor, &cloves[0], &proxy).await {
                Ok(maybe_new_proxy) => {
                    if let Some(new_proxy) = maybe_new_proxy {
                        proxy = self.update_proxy(proxy, new_proxy);
                    }
                }
                Err(_) => return self.handle_alt_forward_failure(&proxy, &second_neighbor),
            }
        }

        if send_error_neighbor2(forward_type) {
            match self.process_neighbor_failure(&second_neighbor, &cloves[1], &proxy).await {
                Ok(maybe_new_proxy) => {
                    if let Some(new_proxy) = maybe_new_proxy {
                        proxy = self.update_proxy(proxy, new_proxy);
                    }
                ...
                Err(_) => self.handle_alt_forward_failure(&proxy, &first_neighbor),
            }
        } else {
            Some(proxy)
        }
    ...
    ///
    /// # Parameters
    /// - `self`: A mutable reference to the current instance of the implementing type.
        ...
        match task.await {
            Ok(result) => match result {
                ...
                Err((code, proxy, cloves, proxy_id)) => {
                    if let Some(changed) = self.handle_proxy_forwarding(code, proxy, cloves).await {
                        proxy_request.proxies.push(changed.clone());
                        proxy_request.proxy_id_associations.insert(proxy_id, changed);
                        Ok(())
                    } else {
