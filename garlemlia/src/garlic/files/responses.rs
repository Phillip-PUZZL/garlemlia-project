...
pub(crate) trait TraitResponses {
    #[allow(dead_code)]
    fn get_download_responses(garlic: &GarlicCast) -> HashMap<U256, Vec<GarlemliaResponse>>;
    #[allow(dead_code)]
    fn get_file_info_responses(garlic: &GarlicCast) -> HashMap<U256, Vec<GarlemliaResponse>>;
    #[allow(dead_code)]
    fn get_search_responses(garlic: &GarlicCast) -> Vec<FileInfo>;
    async fn file_chunk_downloaded(
...
impl TraitResponses for Responses {
    ...
    fn get_download_responses(garlic: &GarlicCast) -> HashMap<U256, Vec<GarlemliaResponse>> {
        ...
        for (request_id, request_data) in &garlic.requests_as_initiator {
            let responses = request_data
                .responses
                .iter()
                .filter_map(|msg| match msg {
                    CloveMessage::Response { data, .. } => match data {
    ...
    fn get_file_info_responses(garlic: &GarlicCast) -> HashMap<U256, Vec<GarlemliaResponse>> {
        let responses = garlic.requests_as_initiator.clone();
        ...
        for i in responses {
            ...
            for j in i.1.responses {
                match j {
                    CloveMessage::Response { data, .. } => match data.clone() {
    ...
    async fn file_chunk_downloaded(
        ...
        let request_info = garlic.requests_as_initiator.get_mut(&request_id);
