use super::garlemlia_protocol::{GarlemliaFindRequest, GarlemliaResponse, InitialChunkInfo};
use crate::core::constants::SOCKET_FILE_DATA_MAX;
...
) -> Result<GarlemliaFilesTracker, Box<dyn std::error::Error>> {
...
pub async fn load_tracker_file(
...
impl GarlemliaFilesTracker {
    ...
    pub async fn save_tracker(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut file = File::create(format!(
            "{}/tracker.data",
            &self.downloads.file_storage_root_path
        ))
        .await?;
...
pub enum GarlemliaData {
    ...
    Validator {
        id: U256,
        proxy_ids: Vec<U256>,
        proxies: HashMap<U256, SocketAddr>,
    },
    FileName {
        id: U256,
        name: String,
...
impl GarlemliaData {
    ...
    pub fn get_response(&self, request: Option<GarlemliaFindRequest>) -> Option<GarlemliaResponse> {
        match self {
            GarlemliaData::Value { value, .. } => Some(GarlemliaResponse::Value {
                value: value.to_string(),
            }),
            GarlemliaData::Validator {
                proxy_ids, proxies, ..
            } => {
                let mut res = GarlemliaResponse::Validator { proxy: None };
    ...
    pub fn get_chunk_responses(
        &self,
        mut data: Vec<u8>,
        request_id: U256,
    ) -> Option<Vec<GarlemliaResponse>> {
        match self {
