...
pub fn new_settings(
    ...
    let settings_dir = format!(
        ...
        settings_directory
            .clone()
            .unwrap_or(env::current_dir()?.to_str().unwrap().to_string())
    )
    .to_string();

    let settings_dir_binding =
        settings_directory.unwrap_or(env::current_dir()?.to_str().unwrap().to_string());
    ...
    let files_dir = files_directory.unwrap_or(
        format!("{}/garlemlia-files", env::current_dir()?.to_str().unwrap()).to_string(),
    );
...
pub async fn load_settings_file(
