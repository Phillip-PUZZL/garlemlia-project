...
use crate::data::garlemlia_data::{
    load_tracker_file, new_tracker, GarlemliaData, GarlemliaFilesTracker,
};
use crate::data::garlemlia_protocol::GarlemliaMessage;
use crate::data::settings::{load_settings_file, new_settings, Settings};
use crate::files::storage::FileStorage;
use crate::garlic::state::garlic_cast::GarlicCast;
use crate::net::event_loop::run_event_loop;
use crate::net::message_handler::{GMessage, GarlemliaMessageHandler, MessageChannel};
use crate::net::node::Node;
use crate::routing::dispatcher::garlemlia_dispatch::GarlemliaDispatch;
...
pub struct StartContext {
    ...
    pub(crate) data_store: Arc<Mutex<HashMap<U256, GarlemliaData>>>,
    pub(crate) garlic: Arc<Mutex<GarlicCast>>,
    pub(crate) file_storage: Arc<Mutex<FileStorage>>,
    pub(crate) chunk_part_associations: Arc<Mutex<ChunkPartAssociations>>,
    pub(crate) check_processing: Arc<Mutex<ProcessingCheck>>,
...
impl Garlemlia {
    pub async fn new(
        ...
        let tracker = new_tracker(settings.get_application_settings().get_root_storage_path())
            .await
            .unwrap();
        ...
        settings.get_node_settings_mut().set_private_key(Some(
            private_key
                .to_pkcs8_pem(LineEnding::LF)
                .unwrap()
                .to_string(),
        ));
        settings.get_node_settings_mut().set_public_key(Some(
            public_key
        ...
        let garlic = GarlicCast::new(
            Arc::clone(&socket),
            node.clone(),
            Arc::new(msg_handler.clone()),
            vec![],
            Some(public_key),
            Some(private_key),
        );
    ...
    pub async fn load(settings_path: String) -> Self {
        ...
        let tracker =
            load_tracker_file(settings.get_application_settings().get_tracker_file_path())
                .await
                .unwrap();
        ...
        for node in settings.get_network_settings().get_known_nodes() {
            rt.add_node(&Arc::new(msg_handler.clone()), node, &socket)
                .await;
        }
    ...
    pub(crate) async fn get_node(&self) -> Node {
        let node;
        {
    ...
    pub(crate) async fn process_message(
        ...
        match msg.clone() {
            ...
            _ => {
                ...
                let response = GarlemliaDispatch::run_message(
                    self_node.clone(),
                    Arc::clone(&socket),
                    Arc::clone(&message_handler),
                    routing_table,
                ...
                if response.is_some() {
                    if let Err(e) = message_handler
                        .send_no_recv(&socket, self_node.clone(), &src, &response.unwrap())
                        .await
                    {
                        eprintln!("Failed to send response to {}: {:?}", src, e);
                    }
    ...
    pub async fn start(&mut self, orig_socket: Arc<UdpSocket>) {
        ...
        let handle = tokio::spawn(async move {
            run_event_loop(ctx).await;
        });

        *Arc::get_mut(&mut self.join_handle).unwrap() = Some(handle);
    }

    pub(crate) async fn sync_known_nodes(
        routing_table: &Arc<Mutex<RoutingTable>>,
    ...
    pub async fn stop(&self) {
        ...
        self.socket
            .send_to(
                &*serde_json::to_vec(&GarlemliaMessage::Stop {}).unwrap(),
                &self.receive_addr,
            )
            .await
            .unwrap();
