use crate::garlic::{Clove, CloveData, CloveNode};
...
impl CloveCache {
    ...
    pub fn insert_clove(&mut self, clove: Clove, from: Node) {
        self.cloves
            .insert(clove.sequence_number, CloveData { clove, from });
    }
    ...
    pub fn insert_alt_node(&mut self, node: CloveNode, alt_node: CloveNode) {
        ...
        if try_sequence.is_some() {
            self.alt_to_sequence
                .insert(alt_node.clone(), try_sequence.unwrap());
            self.insert_association(try_sequence.clone().unwrap(), alt_node.clone());
        } else {
            self.alt_to_sequence
                .insert(alt_node.clone(), node.sequence_number);
    ...
    pub fn insert_my_alt_node(&mut self, sequence_number: U256, my_alt_node: CloveNode) {
        self.my_alt_nodes
            .insert(sequence_number, my_alt_node.clone());
    ...
    pub fn update_sequence_number(&mut self, new_sequence_number: U256, clove_node: CloveNode) {
        ...
        match next {
            Ok(next_hop) => {
                match next_hop {
                    Some(next_node) => {
                        let mut new_next_hop_clove_node = CloveNode {
                            sequence_number: new_sequence_number,
                            node: next_node.clone().node,
                        };
                        new_next_hop_clove_node.sequence_number = new_sequence_number;
                        self.insert_next_hop(
                            new_clove_node.clone(),
                            Some(new_next_hop_clove_node.clone()),
                        );
                        self.insert_next_hop(
                            new_next_hop_clove_node.clone(),
                            Some(new_clove_node.clone()),
                        );
                        ...
                        if has_alt {
                            ...
                            println!(
                                "This should not happen: CloveCache::update_sequence_number():1"
                            );
                        ...
                        if has_alt {
                            ...
                            // after the path has already been solidified
                            println!(
                                "This should not happen: CloveCache::update_sequence_number():2"
                            );
                        ...
                        self.insert_updated_association(
                            clove_node.sequence_number,
                            new_sequence_number,
                        );
                    }
                    None => {
                        ...
                        if has_alt {
                            ...
                            // after the path has already been solidified
                            println!(
                                "This should not happen: CloveCache::update_sequence_number():3"
                            );
            ...
            Some(new_clove_node) => {
                ...
                match forward_clove_node_try {
                    Some(forward_clove_node) => {
                        self.next_hop
                            .insert(new_clove_node.clone(), Some(forward_clove_node.clone()));
                        self.next_hop.remove(&forward_clove_node);
                        self.next_hop
                            .insert(forward_clove_node, Some(new_clove_node.clone()));
                    }
                    _ => {
                        self.next_hop.insert(new_clove_node.clone(), None);
                    }
